...
function AuthProvider({children}){
    ...
    useEffect(() =>{
        async function loadStorage(){
            ...
            //     console.log("O USUARIO ESTA LOGADO? "+ !!user)
            //     user ? setLogado(true) : setLogado(false)
            // })
            const storageUser = await AsyncStorage.getItem('Auth_user')

            if(storageUser){
                setDadosUsuario(JSON.parse(storageUser))
                // setLogado(i =>  i = !!storageUser)
                // setLoading(false)
            }
            // setLogado(i =>  i = !storageUser)
            // setLoading(false)
    ...
    function senhaVisivel1(){
        visivel1 ? setVisivel1(false) : setVisivel1(true)
    }
    function senhaVisivel2(){
        visivel2 ? setVisivel2(false) : setVisivel2(true)
    }
    function senhaVisivel3(){
        visivel3 ? setVisivel3(false) : setVisivel3(true)
    }

    //login
    async function login(email, password, limparTextImput){
        setLoadingAuth(true)
        firebase.auth().signInWithEmailAndPassword(email, password)
        .then((value)=>{
            let index = value.user.email.indexOf("@")
            let nome = value.user.email.substring(0,index)
            limparTextImput()
            setDadosUsuario({ "nome": nome, "email": value.user.email})
            storageUser({ "nome": nome, "email": value.user.email })
            setLoadingAuth(false)
            navigation.navigate("Area do Aluno drawer")
        })
        .catch((error)=>{
            Toast.show({
                type: 'error',
                text1: 'Alerta',
                text2: codigoDeError(error.code)
              })
            setLoadingAuth(false)
        })
    }
    //alterar senha
    async function alterarSenha(senhaAtual, senha){
        setLoadingAuth(true)
        const credencial = firebase.auth.EmailAuthProvider.credential(dadosUsuario.email, senhaAtual);
        ...
        .then(async () => {
            ...
            .then(() =>{
                ...
                Toast.show({
                    type: 'success',
                    text1: dadosUsuario.nome,
                    text1: "sua senha foi alterada com sucesso"
                  })
                  navigation.navigate("AreaDoAluno")                
            })
            .catch((error) => {
                Toast.show({
                    type: 'error',
            ...
            // Alert.alert(codigoDeError(error.code))
    ...
    async function esqueciSenha(email){
        ...
        .then(() => {
            ...
            Toast.show({
                type: 'success',
                text1: 'Email de redefinição de senha foi enviado para seu email',
                text2: 'Atenção verificar se o email esta no SPAN.'
              })
        })
        .catch((error)=>{
    ...
    return(
        <Authcontext.Provider value={{logado: !!dadosUsuario,dadosDoUsuario: dadosUsuario,esqueciSenha, visivel1, visivel2, visivel3, loadingAuth, senhaVisivel1, 
        senhaVisivel2, senhaVisivel3, alterarSenha, login, sairDaConta}}>
           {children} 
